...
只有三个角色，不做用户组，也不做细粒度权限。自部署实例的用户数是个位数，一张表能装下的规则才会被真正遵守。控制台的「用户」页面（`/users`）渲染的就是同一张表，方便管理员一边分配角色一边对照。
...
| 执行自检 | 是 | 是 | 否 |
| 修改运行配置 | 是 | 是 | 否 |
| 修改敏感配置：白名单、CORS、下载代理 | 是 | 否 | 否 |
| 管理账号与角色 | 是 | 否 | 否 |
| 创建与恢复备份 | 是 | 否 | 否 |

角色是阶梯而不是集合：`viewer < operator < admin`，所以管理员能做运维员能做的一切，任何端点都不必把三个角色列一遍。

由此引出两个容易让人意外的结论：

- **角色变更在下一次请求就生效。** 主体在每次调用时都从数据库重建，没有缓存需要等待，也不需要把人踢下线。
- **控制台不会按角色隐藏页面。** 任何已登录用户都能打开导航里的每个页面，真正拒绝操作的是服务端。唯一的例外是「用户」页面：它对非管理员隐藏账号列表并禁用按钮，因为一个所有控件都会报错的页面，比一个说明原因的页面更糟。如果你认为某个页面的存在本身就敏感，那就不要发这个账号。
