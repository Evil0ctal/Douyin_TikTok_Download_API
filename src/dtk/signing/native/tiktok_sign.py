...
#: Environment fingerprint the payload reports, taken from a real browser.
#:
#: These two are the only values in the whole payload that the server verifies,
#: and getting them wrong is silent: every endpoint except ``/api/post/item_list/``
#: answers a mis-signed request normally, so the port looked correct for as long
#: as it was only exercised against the others. ``item_list`` answers 200 with an
#: empty body and a ``tt_orcas_res: 1`` header, which reads exactly like a blocked
#: path and cost a long detour before the signature was suspected at all.
#:
#: The earlier values (129 and 14) came from a Node harness driving the genuine
#: SDK against a minimal DOM. A capture from Chrome says 65 and 8, and correcting
#: just these two turns ``item_list`` from 0 bytes into a full payload - with the
#: environment placeholders below, the home-page ``PAGE`` and ``sequence`` 1 all
#: left as they were. The harness, not the port, was what disagreed with the
#: browser: measure these against a live capture, never against a stub.
ENV_CODE: Final = 65
UB_CODE: Final = 8
...
#: Words in the key spliced into every envelope, as :func:`_random_key` draws it.
KEY_WORDS: Final = 12


def unseal(token: str) -> tuple[bytes, tuple[int, ...]]:
    """Recover ``(payload, key)`` from a signature the platform's own page made.

    The envelope hides its key inside its own ciphertext, at a position derived
    from the key and ciphertext bytes - the server has to recover it the same
    way, so the position is discoverable by testing each candidate against that
    relation. Exactly one satisfies it.

    This is the only honest oracle for this module. A Node harness driving the
    real SDK against a stub DOM reports an environment the browser never sends:
    it produced ``env_code`` 129 and ``ub_code`` 14, where a real page sends 65
    and 8, and because the fixtures were generated by that same harness the unit
    tests agreed with the port while the platform did not. Pin constants against
    a capture from a browser, and use this to read it.
    """
...
def unpack_payload(payload: bytes, *, lead_count: bool = False) -> dict[int, bytes]:
    """Split a payload back into its TLV fields. The inverse of :func:`pack_payload`."""
