...
一个放东西的地方。这个项目之所以存在，是因为有人把「某个签名是怎么构造出来的」写了下来；
论坛是为了下一轮这种事 —— 签名、协议、加壳，凡是被拆开过的都算。

免费、无广告，除了显而易见的那几条之外基本没有条款限制。中英双语，注册即可发布，不设门槛。
...
它是只读的。演示请求不写入请求日志和归档库，所以一个公开实例的数据库不会被陌生人撑爆；
关掉演示模式，所有演示会话立即失效，那把 Key 也立刻作废。

**限流：每 10 秒 30 次请求。** 超了会进入 10 秒冷却，期间你的任何请求都会被拒绝。
这是一台共用的实例，它背后的身份池是所有人同时使用时的上限。

演示站的用途是帮你判断要不要自己部署一套。它不是一个可以拿来对接的服务 —— 它可能挂、
可能被重置、也可能被关掉，而且不会提前通知。
